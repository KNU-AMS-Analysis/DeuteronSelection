...
int main(int argc, char* argv[])
{
  ...
  bool          cutTrkChisq;
  bool          cutTofCharge;
  bool          cutTrkHitLayer1;
  bool          cutTrkHitLayer9;
  bool          cutTofCluster;
  bool          cutRichPass;
  bool          cutTofBeta;
...
//  tree->Branch("accCrossingPhiAngle", &accCrossingPhiAngle, "accCrossingPhiAngle/F");
  ...
  tree->Branch("cutTofChisq", &cutTofChisq, "cutTofChisq/O");
  tree->Branch("cutTrkCharge", &cutTrkCharge, "cutTrkCharge/O");
  tree->Branch("cutTrkYHit", &cutTrkYHit, "cutTrkYHit/O");
  tree->Branch("cutTrkChisq", &cutTrkChisq, "cutTrkChisq/O");
  tree->Branch("cutTofCharge", &cutTofCharge, "cutTofCharge/O");
  tree->Branch("cutTrkHitLayer1", &cutTrkHitLayer1, "cutTrkHitLayer1/O");
  tree->Branch("cutTrkHitLayer9", &cutTrkHitLayer9, "cutTrkHitLayer9/O");
  tree->Branch("cutTofCluster", &cutTofCluster, "cutTofCluster/O");
  tree->Branch("cutRichPass", &cutRichPass, "cutRichPass/O");
  tree->Branch("cutTofBeta", &cutTofBeta, "cutTofBeta/O");
...
//    productionSteps |= Analysis::FillTrdQt;
    ...
    */
    ...
    cutTrkChisq = true;
    cutTofCharge = true;
    cutTrkHitLayer1 = true;
    cutTrkHitLayer9 = true;
    cutTofCluster = true;
    cutRichPass = false;
    cutTofBeta = true;

    if(tofReducedChisqT > 10 || tofReducedChisqC > 10) cutTofChisq = false;
    
    if(trkInnerCharge <= 0.7 || trkInnerCharge > 1.5) cutTrkCharge = false;
    
    if(pParticle->pTrTrack()->GetNhitsY() < 5) cutTrkYHit = false;
    
    if(trkReducedChisquareInnerY >= 10) cutTrkChisq = false;
    
    if(tofLowerCharge <= 0.5 || tofLowerCharge >= 2.0) cutTofCharge = false;
    if(tofUpperCharge <= 0.5 || tofUpperCharge >= 2.0) cutTofCharge = false;

    if(!(pParticle->pTrTrack()->TestHitLayerJ(1))) cutTrkHitLayer1 = false;
    if(!(pParticle->pTrTrack()->TestHitLayerJ(9))) cutTrkHitLayer9 = false;

    Int_t b[4] = {0,0,0,0};

    if(pBeta->NTofClusterH()==4)
    {
      for(Int_t j=0 ; j<pBeta->NTofClusterH() ; j++)
      {
        Int_t tof_layer = pBeta->pTofClusterH(j)->Layer;
      ...
      if(hist_fill != 4) cutTofCluster = false;
    }

    if(pBeta->NTofClusterH()<4) cutTofCluster = false;
    if(nTofClustersInTime > 2) cutTofCluster = false;

    if(richRing)
    {
      if(richRing->IsGood() && richRing->IsClean())
      {
        if(richRing->getProb()>0.2 && richRing->getUsedHits()>2 && richRing->getExpectedPhotoelectrons()>2 && richRing->getPhotoelectrons()>(0.5*richRing->getExpectedPhotoelectrons())) cutRichPass = true;
        if(richRing->getBeta()>1) cutRichPass = false;
      }
    }

    if(tofBeta<0.3) cutTofBeta = false;
